...
from local_scribe.char import char_settings_writer
...
class CLITests(unittest.TestCase):
    ...
    def _run(self, stdin_text: str, *, cwd: Path) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, "-m", "local_scribe.char.char_settings_writer"],
            input=stdin_text,
            text=True,
            capture_output=True,
            cwd=str(cwd),
    ...
    def test_with_launch_id_attaches_suffix(self):
        # 4-line stdin form: writer attaches the .ls<short_id>
        # suffix to the saved token so the ASR server's launch-
        # session gate refuses it after `./run.sh stop`.
        with tempfile.TemporaryDirectory() as td:
            tdp = Path(td)
            sp = tdp / "settings.json"
            sp.write_text("{}")
            launch_id = "deadbeefcafef00d" + "0" * 16
            stdin_text = (
                f"{sp}\n8000\nls_asr_token\n{launch_id}\n"
            )
            proc = self._run(stdin_text, cwd=tdp)
            self.assertEqual(proc.returncode, 0, proc.stderr)
            d = json.loads(sp.read_text())
            saved = d["ai"]["stt"]["openai"]["api_key"]
            self.assertTrue(saved.startswith("ls_asr_token.ls"))
            self.assertTrue(saved.endswith("deadbeefcafef00d"))

    def test_api_with_launch_id_attaches_suffix(self):
        with tempfile.TemporaryDirectory() as td:
            p = Path(td) / "settings.json"
            p.write_text("{}")
            char_settings_writer.patch_settings(
                p, "8000", "ls_asr_test",
                launch_id="1234567890abcdef" + "0" * 16,
            )
            d = json.loads(p.read_text())
            saved = d["ai"]["stt"]["openai"]["api_key"]
            self.assertTrue(saved.startswith("ls_asr_test.ls"))
