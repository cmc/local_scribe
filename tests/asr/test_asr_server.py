...
# After the package refactor parakeet_backend lives at
# ``local_scribe.asr.backends.parakeet_backend``; stub the new path so
# asr_server's ``from local_scribe.asr.backends.parakeet_backend import …``
# picks up our mock instead of trying to load real MLX weights.
_PARAKEET_KEY = "local_scribe.asr.backends.parakeet_backend"
_real_parakeet = sys.modules.get(_PARAKEET_KEY)
...
sys.modules[_PARAKEET_KEY] = _parakeet_stub

from local_scribe.asr import asr_server  # noqa: E402
import local_scribe.asr.backends as _backends_pkg  # noqa: E402  (for diarization stubs)
...
# ``from local_scribe.asr.backends.parakeet_backend import …``, so
# restoring the module here doesn't affect this file's own tests.
if _real_parakeet is not None:
    sys.modules[_PARAKEET_KEY] = _real_parakeet
else:
    sys.modules.pop(_PARAKEET_KEY, None)
...
class AttachSpeakersTests(unittest.TestCase):
    ...
    def test_remaps_sherpa_labels_to_dense_speaker_n(self):
        ...
        with mock.patch.dict(sys.modules, {"local_scribe.asr.backends.diarization_backend": fake_dz}), mock.patch.object(_backends_pkg, "diarization_backend", fake_dz, create=True):
    ...
    def test_long_audio_auto_skips_diarization(self):
        ...
        with mock.patch.object(asr_server, "_MAX_DIARIZE_SECONDS", 1800), \
             mock.patch.dict(sys.modules, {"local_scribe.asr.backends.diarization_backend": fake_dz}), mock.patch.object(_backends_pkg, "diarization_backend", fake_dz, create=True):
            out, n = asr_server._attach_speakers_to_words(
...
class AsrServerAuthIntegrationTests(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        from local_scribe.security import service_auth as sa
