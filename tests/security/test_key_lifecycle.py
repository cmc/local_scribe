...
# ---------- safety wiring (preflight backup + physical presence) ----


class SafetyHardeningTests(_LifecycleBase):
    """Every destructive ``key_lifecycle`` op MUST write a pre-flight
    snapshot before mutating any state. These tests cover each path.
    The dedicated ``key_safety`` mechanics live in
    ``tests/test_key_safety.py``; here we just verify the wiring."""

    def _snapshot_labels(self) -> list[str]:
        return [b.label for b in self.key_safety.list_backups()]

    def test_rotate_writes_preflight_snapshot(self):
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        self.assertNotIn("rotate", self._snapshot_labels())
        result = self.key_lifecycle.rotate_master_key()
        try:
            self.assertIn("rotate", self._snapshot_labels())
        finally:
            result.old.forget()
            result.new.forget()

    def test_add_yubikey_writes_yk_only_snapshot(self):
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(enroll_yubikey=False)
        self.key_lifecycle.add_yubikey(self.SECONDARY)
        snaps = self.key_safety.list_backups()
        self.assertTrue(any(b.label == "add-yubikey" for b in snaps))
        addk_snap = next(b for b in snaps if b.label == "add-yubikey")
        # YK_HALF_ONLY scope → no Keychain backup account.
        self.assertEqual(addk_snap.scope.value, "yk_half_only")
        self.assertIsNone(addk_snap.kc_half_backup_account)

    def test_init_force_writes_preflight_snapshot_when_v2_present(self):
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        self.assertNotIn("init-force", self._snapshot_labels())
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, force=True, dr_passphrase="dr2",
        )
        self.assertIn("init-force", self._snapshot_labels())

    def test_init_first_time_no_snapshot(self):
        # Fresh install — nothing to preserve, so no snapshot.
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        self.assertEqual(self.key_safety.list_backups(), [])

    def test_migrate_writes_v1_legacy_snapshot(self):
        self._preenroll_primary()
        self.secret_store.store_master_key(b"\xd1" * 32)
        self.key_lifecycle.migrate_v1_to_v2()
        snaps = self.key_safety.list_backups()
        self.assertTrue(any(b.label == "migrate-v1-to-v2" for b in snaps))
        snap = next(b for b in snaps if b.label == "migrate-v1-to-v2")
        self.assertEqual(snap.scope.value, "v1_legacy")
        self.assertIn("v1_keychain_account", snap.artefacts)

    # --- dr_restore -----------------------------------------------------

    def test_dr_restore_refuses_to_overwrite_without_explicit_flag(self):
        """When a live v2 install is on the machine, dr-restore MUST
        refuse to re-init unless the caller opts in. Silent overwrite
        would strand any data encrypted under the live key."""
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        with self.assertRaises(RuntimeError) as ctx:
            # Default re_init_yubikey=True + live v2 present + no
            # overwrite flag → refuse.
            self.key_lifecycle.dr_restore("dr")
        self.assertIn("overwrite_existing_v2", str(ctx.exception))

    def test_dr_restore_with_overwrite_writes_snapshot(self):
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        mk = self.key_lifecycle.dr_restore("dr", overwrite_existing_v2=True)
        try:
            self.assertIn(
                "dr-restore-overwrite", self._snapshot_labels(),
            )
        finally:
            mk.forget()

    def test_dr_restore_skips_overwrite_check_when_no_live_v2(self):
        # Fresh disk: DR file exists in isolation, no live v2.
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        # Wipe live v2 to mimic a lost-machine scenario.
        self.secret_store.delete_kc_half()
        self.yubikey_backup.YK_HALF_PATH.unlink()
        # Reinstate the recipient + identity so re-init can wrap.
        self.yubikey_backup.set_recipients([self.PRIMARY])
        _make_identity(self.yubikey_backup.IDENTITY_PATH, self.PRIMARY)
        # Should proceed without the overwrite flag (no v2 to strand).
        mk = self.key_lifecycle.dr_restore("dr")
        try:
            self.assertIsNotNone(mk.as_bytes())
        finally:
            mk.forget()

    # --- destroy CLI ----------------------------------------------------

    def test_destroy_cli_writes_snapshot_by_default(self):
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        rc = self.key_lifecycle._cli_destroy([])  # noqa: SLF001
        self.assertEqual(rc, 0)
        self.assertIn("destroy", self._snapshot_labels())
        # And the live key material is gone.
        self.assertFalse(self.secret_store.has_kc_half())
        self.assertFalse(self.yubikey_backup.has_yk_half())

    def test_destroy_cli_no_backup_skips_snapshot(self):
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        rc = self.key_lifecycle._cli_destroy(  # noqa: SLF001
            ["--no-backup", "--no-presence"]
        )
        self.assertEqual(rc, 0)
        self.assertNotIn("destroy", self._snapshot_labels())

    def test_destroy_cli_requires_physical_presence_by_default(self):
        """Without --no-presence, destroy MUST refuse if the YubiKey
        can't tap (we corrupt yk_half.age to simulate)."""
        self._preenroll_primary()
        self.key_lifecycle.init_master_key(
            enroll_yubikey=False, dr_passphrase="dr",
        )
        ...
        # Key material survived because destroy refused.
        self.assertTrue(self.secret_store.has_kc_half())
