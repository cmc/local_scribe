...
BIN_RECIPES: Dict[str, str] = {
    "age": (
        ...
        "#\n"
        "# Version sourcing precedence:\n"
        "#   1. If LOCAL_SCRIBE_FAKE_AGE_VERSION_FILE points at a readable\n"
        "#      file, the first line is the version string. This is how\n"
        "#      tests model an in-place upgrade: the fake ``brew upgrade\n"
        "#      age`` rewrites the file, and subsequent ``age --version``\n"
        "#      calls (from the post-upgrade re-verify in ensure_age_tools)\n"
        "#      see the new value.\n"
        "#   2. Else LOCAL_SCRIBE_FAKE_AGE_VERSION env var.\n"
        "#   3. Else fixed default 1.2.0 (mirrors a current brew bottle).\n"
        "if [[ \"${1:-}\" == \"--version\" ]]; then\n"
        "  if [[ -n \"${LOCAL_SCRIBE_FAKE_AGE_VERSION_FILE:-}\" \\\n"
        "        && -r \"${LOCAL_SCRIBE_FAKE_AGE_VERSION_FILE}\" ]]; then\n"
        "    head -n1 \"$LOCAL_SCRIBE_FAKE_AGE_VERSION_FILE\"\n"
        "  else\n"
        "    echo \"${LOCAL_SCRIBE_FAKE_AGE_VERSION:-1.2.0}\"\n"
        "  fi\n"
        "  exit 0\n"
        "fi\n"
    ...
    "brew": (
        ...
        "  install|reinstall|update|cleanup) exit 0 ;;\n"
        "  upgrade)\n"
        "    # Model an in-place upgrade: when asked to upgrade ``age``\n"
        "    # AND the operator set LOCAL_SCRIBE_FAKE_BREW_UPGRADE_FLIP,\n"
        "    # rewrite the version file the fake ``age --version`` reads.\n"
        "    # The fake ``age`` then reports the new version on its very\n"
        "    # next invocation, which is exactly what real brew does.\n"
        "    if [[ \"${2:-}\" == \"age\" \\\n"
        "          && -n \"${LOCAL_SCRIBE_FAKE_BREW_UPGRADE_FLIP:-}\" \\\n"
        "          && -n \"${LOCAL_SCRIBE_FAKE_AGE_VERSION_FILE:-}\" ]]; then\n"
        "      echo \"$LOCAL_SCRIBE_FAKE_BREW_UPGRADE_FLIP\" \\\n"
        "          > \"$LOCAL_SCRIBE_FAKE_AGE_VERSION_FILE\"\n"
        "    fi\n"
        "    exit 0\n"
        "    ;;\n"
        "  list) echo \"\"; exit 0 ;;\n"
