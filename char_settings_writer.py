...
def patch_settings(settings_path: pathlib.Path, port: str, token: str) -> None:
    ...
    # the Keychain master key — see service_auth.py). Anything else
    # here would yield a 401 from the ASR server.
    oai["api_key"]          = token
...
def main() -> int:
    ...
    if not settings_path.is_file():
        ...
        return 1
    try:
        patch_settings(settings_path, port, token)
