...
[`§ 14 Threat model × defense layers`](ARCHITECTURE.md#14-threat-model--defense-layers).
...
**Workaround we ship:** [`char_persist.py`](../local_scribe/char/char_persist.py) detects
...
Sentry and the auto-updater have **no in-app toggle**. This repo
ships a **per-Char outbound firewall** by default — it filters only
Char's traffic, not the entire machine.
...
trade-offs (including why we can't use `pf` or the macOS
...
If you have **another** workflow that consumes the same providers
(e.g. you use OpenAI's API from a separate app) and want machine-
wide hardening anyway, opt into the legacy `--mode system`:
...
The list is marker-delimited (`>>> local_scribe firewall` /
`<<< local_scribe firewall`) inside `/etc/hosts` so add / remove
doesn't disturb your existing entries; every change is backed up
to `/etc/hosts.local_scribe.bak.<timestamp>` before being applied
atomically via `rename(2)`.

#### Per-process granularity via third-party tools

If you want belt-and-suspenders on top of our per-Char proxy:

- **[Little Snitch](https://www.obdev.at/products/littlesnitch/index.html)**:
  add deny rules for `char` + `char-cli` matching the hostnames in
  `firewall.BLOCK_CATALOG` (paste `./run.sh firewall list` output
...
- **[LuLu](https://objective-see.org/products/lulu.html)** (free):
  ...
  `char-cli` only. Catches Dock-launch bypasses our wrapper doesn't.
...
list` or [`firewall.py`](../local_scribe/egress/firewall.py) for the canonical version.
...
removes defenses, and when deciding whether a feature request fits
...
The defense isn't airtight (see row #6 above), but the bar moved
...
- ✅ **Per-service bearer tokens** (Keychain + Touch ID). Closes
  tier #4 (shell-as-user). See [`service_auth.py`](../local_scribe/security/service_auth.py) +
  [SECURITY.md § Defense layer 2](../SECURITY.md#defense-layer-2--inter-service-authentication).
- ✅ **`/etc/hosts` block-list manager** for Char's Sentry / PostHog
  / auto-updater + every external STT/LLM provider. Closes the
  "no in-app toggle" gap. See [`firewall.py`](../local_scribe/egress/firewall.py) +
  [SECURITY.md § Defense layer 1](../SECURITY.md#defense-layer-1--network-egress-firewall).
