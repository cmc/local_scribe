...
| `firewall.py` | Block catalog + dual-mode firewall manager. Default `process` mode enforcement lives in `egress_proxy.py` (wired into Char via `HTTPS_PROXY` at launch time). Opt-in `system` mode rewrites `/etc/hosts` machine-wide. Exposes `is_blocked()` for cross-module use. Driven by `./run.sh firewall …`. | Outbound egress control (policy) |
| `egress_proxy.py` | Pure-stdlib asyncio CONNECT proxy on `127.0.0.1:8889`. Refuses CONNECT requests for any host in `firewall.BLOCK_CATALOG` with a `403` + JSON deny body. Ring-buffer audit log. CLI: `python -m egress_proxy {start,status,verify,recent}`. Auto-starts from `./run.sh start`. | Outbound egress control (enforcement) |
| `char_sandbox.py` | Renders the SBPL `sandbox-exec` profile at `~/.config/local_scribe/char.sb`. `(allow default)` + `(deny network-outbound)` + re-allow loopback only. **Not applied at launch** since 2026-05-12 (the wrapper put a terminal at the head of the TCC attribution chain and broke system audio capture); kept for manual operator use. See `docs/CHAR_REVIEW.md` § Layered firewall trade-offs. | Outbound egress control (informational; manual operator opt-in) |
| `char_tcc_probe.py` | Reads recent `tccd` unified-log events and classifies the most-recent `AttributionChain` for `com.hyprnote.stable`. Reports `ok` when Char.app is its own responsible bundle, `terminal` when a terminal emulator slipped into the responsible slot (the May 2026 regression signature). Surfaced in `./run.sh char firewall-status` and the inspector audit panel. | TCC-attribution regression net |
